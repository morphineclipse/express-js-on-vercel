    ...
    tickets.forEach(ticket => {
        ...
        if(localStorage.getItem("token")){
            ...
            <div class="flight-content">
                ...
                <div class="flight-date">${ticket.date}</div>
                <div class="flight-price">${ticket.price}</div>
                <a href="/theatre-ticket/${ticket.id}" class="btn btn-primary flight-btn">Выбрать билет</a>
            </div>
            `
        }
...
async function getTickets() {
    const response = await fetch("/theatre-tickets", {
        method: "POST",
        headers: {
    ...
    // Исправление: сохраняем данные в allTickets
    allTickets = data.theatre;
    loadCards(allTickets);
